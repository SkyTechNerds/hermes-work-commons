...
mapfile -t INCLUDE_REFS < <(git diff "$BASE_SHA" "$HEAD_SHA" "${PATHSPEC[@]}" \
  ...
  | sort -u || true)
[ "${#INCLUDE_REFS[@]}" -eq 0 ] && { emit skip "Keine include-Änderungen im Diff"; exit 0; }
MISSING=""
...
if [ -n "$MISSING" ]; then
  ...
  emit fail "Fehlende include-Ziele:$MISSING"
else
  emit pass "Alle include-Referenzen aufgelöst"
fi
