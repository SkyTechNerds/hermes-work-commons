...
[ "${#PY_FILES[@]}" -eq 0 ] && { emit skip "Keine Python-Dateien im Diff"; exit 0; }
ERRS=0; CHECKED=0
...
[ "$CHECKED" -eq 0 ] && { emit skip "Nur gelöschte Python-Dateien im Diff"; exit 0; }
if [ "$ERRS" -eq 0 ]; then
  emit pass "$CHECKED Python-Datei(en) kompilieren sauber"
else
  emit fail "Syntaxfehler in $ERRS von $CHECKED Python-Datei(en)"
fi
