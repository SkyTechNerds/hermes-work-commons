...
# Check-Modul: phpcs — WordPress Coding Standards, ZWEI-PASS (nur neue/geaenderte Zeilen,
# keine Legacy-Schuld). Inline pro Fund, gedeckelt. Env: BASE_SHA, HEAD_SHA, DIFF_FILES,
# CM_INLINE. cwd=REPO_DIR. Standard/Bin ueberschreibbar via PHPCS_STANDARD/PHPCS_BIN.
...
PHPCS="$PHPCS" STANDARD="${PHPCS_STANDARD:-WordPress}" BASE_SHA="${BASE_SHA:-}" HEAD_SHA="${HEAD_SHA:-}" \
CM_INLINE="${CM_INLINE:-}" CODEMOLE_LANG="${CODEMOLE_LANG:-de}" \
python3 - "${PHP_FILES[@]}" <<'PY'
import os, sys, re, json, subprocess
phpcs = os.environ["PHPCS"]; std = os.environ["STANDARD"]
...
for f in lint_files:
    ...
    add = added_lines(f)
    try:
        r = subprocess.run([phpcs, "--standard=" + std, "--report=json", f],
...
if checked == 0:
    ...
    print(json.dumps({"name": "phpcs", "status": "pass",
                      "message": ((f"WPCS sauber auf neuen Zeilen ({checked} Datei(en))" if de
                                   else f"WPCS clean on new lines ({checked} file(s))") + note)}))
else:
    shown = min(n, MAX_FIND)
    if de:
        m = f"WPCS: {n} Verstoß(e) auf neuen Zeilen in {checked} Datei(en) — {shown} inline markiert{note}"
    else:
        m = f"WPCS: {n} violation(s) on new lines in {checked} file(s) — {shown} flagged inline{note}"
